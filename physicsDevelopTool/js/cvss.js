...
(function(window){
	...
	Canvas.prototype.target=function(name){
		...
		canvas.innerHTML="캔버스를 지원하지않는 브라우저입니다.<br/>최신브라우저로 업데이트해주세요.<br/>참고로 익스플로러 보다는 크롬브라우저를 강력히 추천합니다."
		...
		if(context){
			...
			this.$systemValue={
				...
				boundingBox:{},
				eventDetectionFunction:undefined
			}
		}else{
	...
	Canvas.prototype.detectMOUSEMOVE=function(event){
		...
		for(i=this.$elementKey.length-1; i>=0; i--)
		{
			...
			if(insideCheck){
				...
				//detection function(type,target) => type:mousedown or mouseup , target:insideCheck element
				if(sys.eventDetectionFunction){
					sys.eventDetectionFunction.apply(this,[e,key])
				}
		...
		if(sys.eventDetectionFunction){
			sys.eventDetectionFunction.apply(this,[e])
		}
	...
	Canvas.prototype.detectMOUSEDU=function(event,type){
		...
		for(i=this.$elementKey.length-1; i>=0; i--){
			...
			if(insideCheck){
				...
				if(type=="mousedown" && sys.prevEventElement!=key){
					clearTimeout(sys.dblclickTimerFunc);
					sys.dblclickTimer=false;
				}					
				if(sys.eventFunc[key] && typeof sys.eventFunc[key][type] == "function"){sys.eventFunc[key][type].apply(this,[e,key]);}
				else if(element.baseEventElement && sys.eventFunc[element.baseEventElement] && typeof sys.eventFunc[element.baseEventElement][type]=="function"){sys.eventFunc[element.baseEventElement][type].apply(this,[e,key]);}
			
				if(type=="mouseup" && sys.prevEventElement==key && sys.prevEventType=="mousedown"){
					type="click";
	...
	Canvas.prototype.getRealMousePosition=function(event){
		...
		var e=event.originalEvent || event,
			canvas=event.currentTarget || event.srcElement,
	...
	Canvas.prototype.drawElement=function (info,/*optional*/option){
		...
		for(var i=0; i<K.length; i++){
			...
			var opacity=numberFormat(E.opacity,($E && $E.opacity),initElement.opacity);
			E.opacity=opacity;
			...
			if(typeof E.x=="string" && (E.x.substring(0,2)=="+=" || E.x.substring(0,2)=="-=")){
				E.x=x;
			}
			if(typeof E.y=="string" && (E.y.substring(0,2)=="+=" || E.y.substring(0,2)=="-=")){
				E.y=y;
			}			
			switch(E.type){
				case "block":
					...
					if(typeof E.width=="string" && (E.width.substring(0,2)=="+=" || E.width.substring(0,2)=="-=")){
						E.width=width;
					}
					if(typeof E.height=="string" && (E.height.substring(0,2)=="+=" || E.height.substring(0,2)=="-=")){
						E.height=height;
					}				
					if(E.minWidth){
	...
	Canvas.prototype.remove=function(){
		...
		this.$elementKey.splice(this.$elementKey.indexOf(target),1);
	...
	Canvas.prototype.click=function(func){this.submitEvent("click",func);}
	...
	//캔버스내에서 발생하는 모든 이벤트를 감지한다
	Canvas.prototype.eventDetection=function(func){
		if(typeof func=="function"){
			this.$systemValue.eventDetectionFunction=func;
			return 1;
		}
		return;
	}
	...
	Canvas.prototype.unbind=function(events){
		...
		for(i=0; i<events.length; i++){
			if(events[i]=="hover"){
				delete sys.eventFunc[target]["mousein"];
				delete sys.eventFunc[target]["mouseout"];
			}
