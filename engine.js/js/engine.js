...
(function(window){
	window.Engine=function(canvas,engineInfo){
		...
		this.$gravity=(engineInfo && !isNaN(engineInfo.gravity) && [0,engineInfo.gravity]) || Engine.prototype.basicEngineInfo.gravity; // 중력가속도 설정
	...
	Engine.prototype.$gearSystem=function(){
		...
		if(sysInfo.delta>sysInfo.engineTime){
			...
			for(i=0; i<particleKey.length; i++){
				...
				/*적분기*/
				if(this.integrator(this.$particle[particleKey[i]],sysInfo.engineTime) || this.$particle[particleKey[i]].updatePosition){
					/*위치 업데이트*/
	...
	Engine.prototype.integrator=function(particle,duration){
		...
		var on=Object.keys(particle.onParticle).length>0;
		...
		var gravity=(particle.applyGravity && !on) ? this.$systemValue.gravity : [0,0];
	...
	Engine.prototype.particle=function(key,info){
		...
		if(!info.style && !this.$particle[key]){
			info.style=key;
		}	

		var newParticle=this.$particle[info.style] ? false : true;
		...
		switch(newParticle){
			case true:
				...
				info.onParticle=new Object();
				info.pushOn=new Object();
	...
	/*tag기능*/
	Engine.prototype.addTag=function(tag){
		if(this.$particleStack.length<1){
			return
		}
		var particleName=this.$particleStack.pop();
		var particle=this.$particle[particleName];
		if(particle.tag){
			this.particle(particleName).removeTag();
		}

		this.$particle[particleName].tag=tag;

		var list=this.$systemValue.tagList;
		if(!list[tag]){
			list[tag]=[]
		}
		list[tag].push(particleName);
	}

	Engine.prototype.removeTag=function(){
		if(this.$particleStack.length<1){
			return
		}
		var particleName=this.$particleStack.pop();
		...
		if(particle.tag){
			var index=list[particle.tag].indexOf(particleName)
			list[particle.tag].splice(index,1);
			delete particle.tag;
			if(list[particle.tag].length==0){
				delete list[particle.tag];
			}
			return 1;
		}
	...
	Engine.prototype.tagList=function(tag){
		var list=this.$systemValue.tagList[tag];
		if(list){
			return Canvas.prototype.CopyObj(list);
		}
		return list;
	}
	//particle.property('f',particle.proeprty('f')+nV); 을 통하여 힘발생기 같은 역할이 가능하다.
	//property속성을 통해 입자의 속성이 변경될수 있는데, applyParticleProperty함수는  적분기 실행전에 property속성을 사용해 등록되어진 변경사항들을 일괄적으로 처리한다 
		...
		var obj=this.$systemValue.standbyParticleProperty,
			...
			for(var i=0; i<key.length; i++){
				...
				for(var j=0; j<subKey.length; j++){
					if(subKey[i]=="s"){
						this.$particle[key[i]].updatePosition=true;
					}
					if(subKey[i]=="v"){
						this.$particle[key[i]].updateVelocity=true;
					}
	...
	Engine.prototype.property=function(propertyName,propertyValue){
		...
		var particleName=this.$particleStack.pop(),
			particle=this.$systemValue.standbyParticleProperty;
	...
	Engine.prototype.collisionDetection=function(duration){
		for(var i=0; i<this.$particleKey.length; i++){
			var target=this.$particleKey[i];
			for(var j=i+1; j<this.$particleKey.length; j++){
				var subTarget=this.$particleKey[j];
				if(this.REALcollisionDetection(target,subTarget,duration)){
				}else{
					this.removeParticledata(this.$particle[target].onParticle,subTarget);
					this.removeParticledata(this.$particle[subTarget].onParticle,target);
					
					this.removeParticledata(this.$particle[target].pushOn,subTarget);
					this.removeParticledata(this.$particle[subTarget].pushOn,target);
				}
			}
			if(this.$particle[target].updatePosition===true){
				delete this.$particle[target].updatePosition;
			}
			if(this.$particle[target].updateVelocity===true){
				delete this.$particle[target].updateVelocity;
			}
		}
		return;
	}
	Engine.prototype.REALcollisionDetection=function(t,o,duration){
		...
		var hasSpeed={
			t:[Math.abs(tProperty.v[0])>0.8660254037844386,Math.abs(tProperty.v[1])>5],
			o:[Math.abs(oProperty.v[0])>0.8660254037844386,Math.abs(oProperty.v[1])>5]
		}

		if(!oProperty.updateVelocity && !tProperty.updateVelocity && !tProperty.updatePosition && !oProperty.updatePosition){//사용자가 임의로 속도를 지정했을 경우
			if((!hasSpeed.o[1] && oProperty.onParticle[t]) || (!hasSpeed.t[1] && tProperty.onParticle[o])){ //정지(에 근접한) 물체 끼리 충돌했을 경우 얼리아웃
				//rest 상태라고 지칭하자
				return 1;
			}
			if((hasSpeed.t[0] && !hasSpeed.t[1] && tProperty.onParticle[o]) || (hasSpeed.o[0] && !hasSpeed.o[1] && oProperty.onParticle[t])){ //지표면과 수직으로 이동할경우 얼리아웃
				return 1;
			}
		}
		...
		for(var i=0; i<MAX_iteration; i++){ // 몇몇 무한루프 때문에 MAX_intertation 제약을 주었다
			...
			if(d-edge.distance<0.1){
				collisionData.normal=edge.normal;
				if(edge.normal[0]==0 && edge.normal[1]==0){
					return 1;
				}
	...
	Engine.prototype.removeParticledata=function(obj,name){
		if(obj[name]){
			delete obj[name];
		}
		return;
	...
	Engine.prototype.collisionProcessing=function(particle,duration,contactData){ //충돌 처리
		...
		var tempMass={
			A:A.inverseMass,
			B:B.inverseMass,
			total:A.inverseMass+B.inverseMass
		}
		if(!this.empty(A.onParticle)){
			A.v[1]=0
			if(B.inverseMass>0){
				A.inverseMass=0;
			}
		}
		if(!this.empty(B.onParticle)){
			B.v[1]=0
			if(A.inverseMass>0){
				B.inverseMass=0;
			}
		...
		if(Math.abs(A.v[1])<5 && contactData.normal[1]==-1){
			A.onParticle[particle[1]]=true;
		}else{
			delete A.onParticle[particle[1]]
		}
		if(Math.abs(B.v[1])<5 && contactData.normal[1]==1){
			B.onParticle[particle[0]]=true;
		}else{
			delete B.onParticle[particle[0]]
		}

		if(contactData.normal[1]==-1){
			B.pushOn[particle[0]]=true; //위에서미는 파티클에 등록
		}else{
			delete B.pushOn[particle[0]]; //위에서미는 파티클에 등록
		}
		if(contactData.normal[1]==1){
			A.pushOn[particle[1]]=true; //위에서미는 파티클에 등록
		}else{
			delete A.pushOn[particle[1]]; //위에서미는 파티클에 등록
		}
		...
		//if(totalInverseMass==0){ //두 입자의 INVERSMASS의 합이 0 인경우는 충돌감지에서 미리 방지를하는데 여기서 둘다 INVERMASS가 0이라는건 아까 ONPARTICLE 일경우 역질량 배경으로 대채한다는 경우밖에없다.
		//	B.inverseMass=tempMass.B;
		//	totalInverseMass=A.inverseMass+B.inverseMass;
		//}
		if(contactData.deph>0 && totalInverseMass>0){
			//역질량을 기준으로하면 여러에러와 연산속도가 빨라진다.
			var totalMove=contactData.normal[1]*(contactData.deph/totalInverseMass);
			A.s[1]+=(totalMove*A.inverseMass);
			B.s[1]-=(totalMove*B.inverseMass);

			var tempTotal=contactData.normal[0]*(contactData.deph/tempMass.total);
			A.s[0]+=(tempTotal*tempMass.A);
			B.s[0]-=(tempTotal*tempMass.B);
		}

		A.inverseMass=tempMass.A;
		B.inverseMass=tempMass.B;
		...
		if(!this.empty(A.pushOn)){
			A.v[1]=0;
		}
		if(!this.empty(B.pushOn)){
			B.v[1]=0;
		}
	...
	Engine.prototype.empty=function(obj){
